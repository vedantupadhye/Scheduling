...
//       const itemData = {
//         outMaterialNo: `NA${String(index + 1).padStart(5, "0")}`,
//         outThickness: inventoryItem.orderThickness || null,
//         outWidth: inventoryItem.orderWidth || null,
//         outGrade: inventoryItem.grade || null,
//         outCoilWeight: inventoryItem.weight || null,
//         inMatNo: inventoryItem.IN_MaterialName,
//         inThickness: inventoryItem.IN_Thickness,
//         inWidth: inventoryItem.IN_Width,
//         inGrade: inventoryItem.IN_Grade,
//         inactualWeight: inventoryItem.ActuallWeight, // Map from Inventory.ActuallWeight
//         orderNo: String(orderNo),
//       };
//       selectedData.push(itemData);
//     });
...
//       router.push(`/schedule-confirmation?scheduleNo=${scheduleNo}`);
//     } else {
//       setError("Failed to save schedule: " + result.error);
//     }
//   };
//   return (
//     <div className="min-h-screen bg-gray-900 py-8 px-4">
...
export default function SchedulePool() {
  ...
  const sortInventory = (inventory, order) => {
    return inventory.sort((a, b) => {
      ...
      const isAboveA = thickA > orderThick ? 0 : 1;
      const isAboveB = thickB > orderThick ? 0 : 1;
      if (isAboveA !== isAboveB) return isAboveA - isAboveB;
  ...
  useEffect(() => {
    async function fetchInitialData() {
      ...
      try {
        const fetchedOrders = await getOrdersByDate(selectedDate);
        const makerResult = await getScheduleMakerItems();
        if (fetchedOrders.length === 0) {
          setError("No orders found for the selected date.");
        }
        const ordersWithStatus = fetchedOrders.map((order) => ({
        ...
        setOrders(ordersWithStatus);
        setScheduleMakerItems(makerResult.success ? makerResult.data : []);
        console.log("Fetched Orders:", ordersWithStatus);
        console.log("Fetched Schedule Maker Items:", makerResult.data);
        setSelectedOrders([]);
        setInventoryMatches({});
        setSelectedInventory({});
        setShowInventory(false);
  ...
  const handleSelectAllScheduleItems = () => {
    if (selectedScheduleItems.length === scheduleMakerItems.length) {
      setSelectedScheduleItems([]); // Deselect all
    } else {
      const allKeys = scheduleMakerItems.map(
        (item, index) => `${item.orderNo}-${item.inMaterialName}-${item.YardNO || "no-yard"}-${index}`
      );
      setSelectedScheduleItems(allKeys); // Select all
    }
  };
  ...
  const handleOrderAllocation = async () => {
    if (selectedOrders.length === 0) {
      setError("Please select at least one order to allocate.");
      return;
    }
    ...
    selectedOrders.forEach((orderNo) => {
      ...
      if (order && selectedInventory[orderNo]?.length > 0) {
        ...
        console.log(`Selected Items for Order ${orderNo}:`, selectedItems);
        if (selectedItems.length > 0) {
          const itemsWithOrderDetails = selectedItems.map((item) => ({
            ...item,
            orderNo: orderNo, // Ensure orderNo is included
            orderThickness: order.orderThickness,
            orderWidth: order.orderWidth,
            grade: order.grade,
            weight: order.weight,
          }));
          newScheduleItems.push(...itemsWithOrderDetails);
          handleStatusChange(
            orderNo,
            selectedItems.length >= order.quantity ? "Allocated" : "Partially Allocated"
          );
        }
      } else {
        console.log(`No selected inventory for Order ${orderNo}`);
      }
    });

    console.log("New Schedule Items to Add:", newScheduleItems);

    if (newScheduleItems.length > 0) {
      try {
        setLoading(true);
        for (const orderNo of selectedOrders) {
          const itemsForOrder = newScheduleItems.filter(
            (item) => String(item.orderNo) === String(orderNo)
          );
          if (itemsForOrder.length > 0) {
            const result = await addToScheduleMaker(orderNo, itemsForOrder);
            console.log(`Add to ScheduleMaker Result for Order ${orderNo}:`, result);
            if (!result.success) {
              setError(`Failed to add items for Order ${orderNo}: ${result.error}`);
            }
          }
        }
        const makerResult = await getScheduleMakerItems();
        console.log("Updated Schedule Maker Items from DB:", makerResult.data);
        if (makerResult.success) {
          setScheduleMakerItems(makerResult.data);
        } else {
          setError("Failed to fetch updated Schedule Maker items: " + makerResult.error);
        }
      } catch (err) {
        setError("Error during allocation: " + err.message);
      } finally {
        setLoading(false);
      }
    } else {
      setError("No items selected to allocate.");
    }
  ...
  const handleCreateSchedule = async () => {
    ...
    selectedScheduleItems.forEach((itemKey, index) => {
      const [orderNo, materialName] = itemKey.split("-");
      const inventoryItem = scheduleMakerItems.find(
        (item) => String(item.orderNo) === orderNo && item.inMaterialName === materialName
      );
      ...
      const itemData = {
        ...
        outGrade: inventoryItem.grade || null,
        outCoilWeight: inventoryItem.weight || null,
        inMatNo: inventoryItem.inMaterialName,
        inThickness: inventoryItem.inThickness,
        inWidth: inventoryItem.inWidth,
        inGrade: inventoryItem.inGrade,
        inactualWeight: inventoryItem.inactualWeight,
        orderNo: String(orderNo),
      };
      selectedData.push(itemData);
    });
    ...
    // Don't get a new schedule number here
    // Instead, store the selectedData in sessionStorage
    sessionStorage.setItem('selectedScheduleData', JSON.stringify(selectedData));
    
    // Pass an empty scheduleNo parameter to let the confirmation page handle it
    router.push(`/schedule-confirmation`, {
      state: { selectedData, fromSchedulePool: true },
    });
    setLoading(false);
  };
  ...
  //   setLoading(true);
  //   const scheduleNoResult = await getNextScheduleNumber();
  //   if (scheduleNoResult.success === false) {
  //     setError("Failed to generate schedule number: " + scheduleNoResult.error);
  //     setLoading(false);
  //     return;
  //   }
  //   const scheduleNo = scheduleNoResult;
  
  //   // Store selectedData in localStorage
  //   localStorage.setItem("scheduleConfirmationData", JSON.stringify(selectedData));
  //   localStorage.setItem("fromSchedulePool", "true");
                ...
                <Table>
                  <TableHeader>
                    <TableRow className="bg-gray-700">
                      <TableHead className="text-white px-4 py-2">
                        <Checkbox
                          checked={
                            selectedScheduleItems.length === scheduleMakerItems.length &&
                            scheduleMakerItems.length > 0
                          }
                          onCheckedChange={handleSelectAllScheduleItems}
                          className="bg-white"
                        />
                  ...
                  <TableBody>
                    {scheduleMakerItems.length === 0 ? (
                      ...
                      scheduleMakerItems.map((item, index) => {
                        const itemKey = `${item.orderNo}-${item.inMaterialName}-${item.YardNO || "no-yard"}-${index}`;
                        ...
                        return (
                          <TableRow key={itemKey} className="hover:bg-indigo-600/20">
                            ...
                            <TableCell className="text-white px-4 py-2">{String(item.orderNo)}</TableCell>
                            <TableCell className="text-white px-4 py-2">{item.inMaterialName}</TableCell>
                            <TableCell className="text-white px-4 py-2">{item.inThickness}</TableCell>
                            <TableCell className="text-white px-4 py-2">{item.inWidth}</TableCell>
                            <TableCell className="text-white px-4 py-2">{item.inactualWeight}</TableCell>
                            <TableCell className="text-white px-4 py-2">{item.inGrade}</TableCell>
